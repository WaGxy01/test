...
class ExecutionTraceAnalyzer(OnTheFlyAnalysis):
    ...
    def register_step(self, g, population, engine):
        self.env.current_generation = g + 1
    ...
    def execution_function(self, indv, env: FuzzingEnvironment):
        is_new = indv.hash not in env.unique_individuals
        env.unique_individuals.add(indv.hash)
    
        if is_new:
            if not hasattr(env, 'unique_individuals_data'):
                env.unique_individuals_data = {}
            env.unique_individuals_data[indv.hash] = {
                "generation": getattr(env, 'current_generation', -1),
    ...
    def finalize(self, population, engine):
        ...
        # ---- 输出所有唯一个体的交易序列 ----
        mapping = getattr(engine, 'mapping', {})  # function hash -> signature

        def _serialize_chromosome(chromosome):
            """把 chromosome 转成可读的交易序列列表"""
            txs = []
            for gene in chromosome:
                func_hash = gene["arguments"][0] if gene.get("arguments") else "unknown"
                # 解析函数名
                if func_hash in mapping:
                    func_name = mapping[func_hash]
                elif func_hash == "constructor":
                    func_name = "constructor"
                elif func_hash == "fallback":
                    func_name = "fallback"
                else:
                    func_name = func_hash

                args = gene["arguments"][1:] if gene.get("arguments") and len(gene["arguments"]) > 1 else []
                txs.append({
                 "function": func_name,
                    "arguments": [str(a) for a in args],
                    "from": gene.get("account", ""),
                    "value": gene.get("amount", 0),
                    "timestamp": gene.get("timestamp"),
                    "blocknumber": gene.get("blocknumber"),
                })
            return txs

        unique_data = getattr(self.env, 'unique_individuals_data', {})
        self.env.results["unique_individuals"] = []
        for h, entry in unique_data.items():
            self.env.results["unique_individuals"].append({
                "hash": h,
                "generation": entry["generation"],
                "transactions": _serialize_chromosome(entry["chromosome"])
            })
        # 按代数排序方便阅读
        self.env.results["unique_individuals"].sort(key=lambda x: x["generation"])
