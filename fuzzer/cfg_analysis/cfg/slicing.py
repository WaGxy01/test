...
from .backward import traverse_back
...
class SlicingState(object):
    def __init__(self, stacksize, stack_underflow, stack_delta, taintmap, memory_taint, backward_slice, instructions):
        ...
        if isinstance(taintmap, frozenset):
            self.taintmap = taintmap
        else:
            self.taintmap = frozenset(taintmap)
    ...
    def __hash__(self):
        ...
        return sum(a * b for a, b in zip((23, 29, 31, 37, 41), (
            self.stacksize, self.stack_delta, len(self.taintmap),
            len(self.instructions), len(self.backward_slice))))
    ...
    for ins in instructions[::-1]:
        ...
        if slice_candidate:
            ...
            if 0x80 <= ins.op <= 0x8f:  # Special handling for DUPa
                ...
                add_to_slice = True
                # 优化：避免创建过大的集合，添加边界检查
                start = stacksize - ins.outs
                end = stacksize
                # 限制范围大小，防止创建巨大集合
                MAX_RANGE_SIZE = 1024  # 合理的栈大小上限
                if end - start > MAX_RANGE_SIZE:
                    # 如果范围太大，只保留实际在taintmap中的元素
                    taintmap = set(x for x in taintmap if not (start <= x < end))
                else:
                    taintmap -= set(range(start, end))

                # 同样处理新添加的范围
                new_start = stacksize - ins.outs
                new_end = stacksize - ins.delta
                if new_end - new_start > MAX_RANGE_SIZE:
                    # 范围太大，跳过添加
                    pass
                else:
                    taintmap |= set(range(new_start, new_end))
...
def backward_slice(ins, taint_args=None, memory_info=None, initial_gas=10, must_visits=[], reachable=False):
    ...
    # 边界检查：EVM 栈最大深度是 1024
    MAX_STACK_SIZE = 1024
    if ins.ins > MAX_STACK_SIZE:
        import logging
        logging.warning(f"异常的栈大小: {ins.ins} at address {ins.addr:x}, 限制为 {MAX_STACK_SIZE}")
        actual_stack_size = MAX_STACK_SIZE
    else:
        actual_stack_size = ins.ins
    
    if taint_args:
        taintmap = set((actual_stack_size - 1) - i for i in taint_args if i < actual_stack_size)
    else:
        taintmap = set(range(actual_stack_size))
...
def interesting_slices(instruction, args=None, memory_info=None, reachable=False, taintedBy=potentially_user_controlled, restricted=True):
    if restricted: 
        return [bs for bs in backward_slice(instruction, args, memory_info, reachable=reachable) if any(
            ins.name in taintedBy for ins in bs)]
    else:
        return [bs for bs in backward_slice(instruction, args, memory_info, reachable=reachable)]

def change_slices(instruction, args=None, reachable=False):    
    return [bs for bs in backward_slice(instruction, args, reachable=reachable)]
