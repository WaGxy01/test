...
from . import utils
from .exceptions import ExternalData, SymbolicError, IntractablePath, VMException
from .results import SymbolicResult, gen_exec_id
from .state import SymRead, EVMState, SymbolicEVMState
from .z3_extra_util import concrete, is_true
...
def run(program, state=None, code=None, ctx=None, check_initialized=False, trace=False):
    ...
    while state.pc in program:
        ...
        # SWAPn (eg. SWAP1: a b c d -> a b d c, SWAP3: a b c d -> d b c a)
            ...
            swap_depth = opcode - 0x8f
            while len(stk) < swap_depth + 1:
                stk.insert(0, f"sym_{len(stk)}")
            stk[-1], stk[-(swap_depth + 1)] = stk[-(swap_depth + 1)], stk[-1]
