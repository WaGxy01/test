...
class BackwardExplorerState(object):
    ...
    def rank(self):
        ...
        :return:
        """
        # return self.estimate(), len(self.must_visit), id(self)

        if self._cached_rank is None:
            self._cached_rank = (self.estimate(), len(self.must_visit), id(self))
...
def traverse_back(start_ins, initial_gas, initial_data, advance_data, update_data, finish_path, must_visits=[],
    ...
    if total_ended:
        logging.debug("%d paths that ended prematurely due to branches: %s", total_ended,
                     ', '.join('%x: %d' % (k, v) for k, v in ended_prematurely.items()))
    else:
        logging.debug("Finished all paths")
