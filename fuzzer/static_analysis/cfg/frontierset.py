...
class FrontierSet(object):
    ...
    def all(self):
        if self._frozenall is None:
            # 使用原始方法但确保安全
            self._frozenall = frozenset(
                set(self._blocking_set.keys()) |
                set(self._inhibiting_set.keys()) |
                self._frontier
            )
        return self._frozenall
    ...
    def copy(self):
        ...
        new._frontier = self._frontier.copy()
        # 复制缓存，避免重新计算
        new._frozenedges = self._frozenedges
        new._frozenfrontier = self._frozenfrontier
        new._frozenall = self._frozenall
